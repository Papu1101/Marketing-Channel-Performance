-- ============================================================
--  CHANNEL PERFORMANCE & METRIC EVALUATION
-- ============================================================

WITH channel_spend AS (
    SELECT
        channel_key,
        SUM(spend) AS total_spend,
        SUM(impressions) AS total_impressions,
        SUM(clicks) AS total_clicks
    FROM ad_spend_final
    GROUP BY channel_key
),
channel_outcomes AS (
    SELECT
        channel_key,
        COUNT(*) AS total_signups,
        COUNT(*) FILTER (
            WHERE subscription_start_date IS NOT NULL
        ) AS paying_subscribers,
        SUM(revenue_first_90_days) AS revenue_first_90_days
    FROM signups_final
    WHERE campaign_key IS NOT NULL
    GROUP BY channel_key
)
SELECT
    s.channel_key,
    ROUND(s.total_spend, 2) AS total_spend,
    s.total_impressions,
    s.total_clicks,
    o.total_signups,
    o.paying_subscribers,
    ROUND(o.revenue_first_90_days, 2) AS revenue_first_90_days,
    ROUND(
        s.total_clicks::NUMERIC
        / NULLIF(s.total_impressions, 0) * 100,
        2
    ) AS ctr_percent,
    ROUND(
        s.total_spend
        / NULLIF(s.total_clicks, 0),
        2
    ) AS cpc,
    ROUND(
        s.total_spend
        / NULLIF(o.total_signups, 0),
        2
    ) AS cost_per_signup,
    ROUND(
        s.total_spend
        / NULLIF(o.paying_subscribers, 0),
        2
    ) AS cost_per_paying_subscriber,
    ROUND(
        o.revenue_first_90_days
        / NULLIF(s.total_spend, 0),
        2
    ) AS roas
FROM channel_spend s
LEFT JOIN channel_outcomes o
    ON s.channel_key = o.channel_key
ORDER BY roas DESC;

--Compare channel rankings----

WITH channel_spend AS (
    SELECT
        channel_key,
        SUM(spend) AS total_spend,
        SUM(impressions) AS total_impressions,
        SUM(clicks) AS total_clicks
    FROM ad_spend_final
    GROUP BY channel_key
),
channel_outcomes AS (
    SELECT
        channel_key,
        COUNT(*) AS total_signups,
        COUNT(*) FILTER (
            WHERE subscription_start_date IS NOT NULL
        ) AS paying_subscribers,
        SUM(revenue_first_90_days) AS revenue_first_90_days
    FROM signups_final
    WHERE campaign_key IS NOT NULL
    GROUP BY channel_key
),
channel_metrics AS (
    SELECT
        s.channel_key,
        s.total_spend,
        s.total_clicks,
        o.total_signups,
        o.paying_subscribers,
        o.revenue_first_90_days,
        s.total_spend
            / NULLIF(s.total_clicks, 0) AS cpc,
        s.total_spend
            / NULLIF(o.total_signups, 0) AS cost_per_signup,
        s.total_spend
            / NULLIF(o.paying_subscribers, 0)
            AS cost_per_paying_subscriber,
        o.revenue_first_90_days
            / NULLIF(s.total_spend, 0) AS roas
    FROM channel_spend s
    JOIN channel_outcomes o
        ON s.channel_key = o.channel_key
)
SELECT
    channel_key,
    ROUND(cpc, 2) AS cpc,
    RANK() OVER (
        ORDER BY cpc ASC
    ) AS cpc_rank,
    ROUND(cost_per_signup, 2) AS cost_per_signup,
    RANK() OVER (
        ORDER BY cost_per_signup ASC
    ) AS cost_per_signup_rank,
    ROUND(cost_per_paying_subscriber, 2)
        AS cost_per_paying_subscriber,
    RANK() OVER (
        ORDER BY cost_per_paying_subscriber ASC
    ) AS cost_per_paying_subscriber_rank,
    ROUND(roas, 2) AS roas,
    RANK() OVER (
        ORDER BY roas DESC
    ) AS roas_rank
FROM channel_metrics
ORDER BY roas_rank;